...
let scoreStars = addCommentForm.querySelectorAll('.score i') ;
let userScore = addCommentForm.querySelector('.score input[type="hidden"]') ;
...
scoreStars.forEach(star => {
    star.addEventListener('mouseenter',fillStar) ;
}) ;
function fillStar(e){
    let index = parseInt(this.getAttribute('data-index')) ;
    scoreStars.forEach((star,i)=>{
        if(i<=index) star.classList.add('fill') ;
        else star.classList.remove('fill') ;
    })
    userScore.value = index+1 ;
}
//each comment -------------------------------------
//each comment -------------------------------------
//each comment -------------------------------------
